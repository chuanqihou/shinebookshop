...
public class UserDaoImpl implements UserDao {
	...
	public boolean delUser(int id) {
		...
		String sql4 = "select * from s_order where userId=?";
		String sql3 = "delete from s_orderitem where orderId=?";
		List<Map<String, Object>> list = DbUtil.executeQuery(sql4,id);
		OrderItem orderItem = null;
		if(list.size()>0) {
			for (Map<String, Object> map:list) {
				DbUtil.excuteUpdate(sql3, map.get("orderId"));
			}
		}
		String sql2 = "delete from s_order where userId=?";
		DbUtil.excuteUpdate(sql2, id);

		String sql1="delete from s_user where userId=?";
		int i=DbUtil.excuteUpdate(sql1, id);
		return i>0?true:false;
	}
	/**
	...
	public boolean batDelUser(String ids) {
		...
		String sql4 = "select * from s_order where userId in ("+ids+")";
		String sql3 = "delete from s_orderitem where orderId=?";
		List<Map<String, Object>> list = DbUtil.executeQuery(sql4);
		OrderItem orderItem = null;
		if(list.size()>0) {
			for (Map<String, Object> map:list) {
				DbUtil.excuteUpdate(sql3, map.get("orderId"));
		...
		String sql2 = "delete from s_order where userId in ("+ids+")";
